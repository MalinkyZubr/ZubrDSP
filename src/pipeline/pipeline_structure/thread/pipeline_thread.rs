...
use crate::pipeline::pipeline_structure::node_enum::PipelineNodeEnum;
use super::thread_diagnostics::{PipelineThreadState, BaseThreadDiagnostic};
...
impl ThreadTapManager {
    ...
    async fn send_diagnostic(state: Arc<RwLock<PipelineThreadState>>, execution_time: Arc<RwLock<f32>>, message_sender: Arc<channel::Sender<BaseThreadDiagnostic>>) {
        while {
            ...
            let _result = timeout(Duration::from_millis(100), async {
                let _result = message_sender.send(
